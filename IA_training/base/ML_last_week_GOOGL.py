...
# Lire la version actuelle à partir du fichier count.txt
version_file = f"model/{name}/count.txt"
...
df['Target'] = df['GOOGL_Close'].shift(-7)
df['Target_Binary'] = (df['GOOGL_Close'].shift(-7) > df['GOOGL_Close']).astype(int)
df = df.dropna()

# Affichage des dernières lignes
print("Dernières lignes des données originales :")
print(df[['GOOGL_Close', 'Target']].tail())
...
if train_model:
    ...
    model = Sequential([
    Dense(256, activation='relu', input_shape=(X_train_scaled.shape[1],)),  # Couche d'entrée avec plus de neurones
    Dropout(0.3),  # Dropout pour éviter l'overfitting
    Dense(128, activation='relu'),  # Deuxième couche cachée
    Dropout(0.3),
    Dense(64, activation='relu'),  # Troisième couche cachée
    Dropout(0.2),
    Dense(32, activation='relu'),  # Quatrième couche cachée
    Dropout(0.2),
    Dense(1, activation='linear')  # Couche de sortie linéaire
    ])

    model.compile(
    optimizer=Adam(learning_rate=0.001),  # Réduction du taux d'apprentissage pour éviter les oscillations
    ...
    monitor='val_loss', 
    patience=15,  # Attendre plus longtemps avant d'arrêter
    restore_best_weights=True)

    history = model.fit(
    X_train_scaled, y_train_scaled,
    validation_split=0.2,
    epochs=200,  # Augmenter le nombre d'epochs pour un modèle plus complexe
    batch_size=32,
    callbacks=[early_stopping],
    verbose=1)

    # Sauvegarde de l'historique
    with open(f"model/{name}/training_history_v{version}.json", "w") as f:
        json.dump(history.history, f)

    print("Historique de l'entraînement sauvegardé avec succès !")
else:
    # Chargement du modèle sauvegardé
    model = load_model(f"model/{name}/trading_model_v{version}.keras")
    print("Modèle chargé avec succès !")

    # Chargement de l'historique
    with open(f"model/{name}/training_history_v{version}.json", "r") as f:
...
model.save(f"model/{name}/trading_model_v{version}.keras")
...
if train_model:
    ...
    # Sauvegarder la nouvelle version dans le fichier count.txt
    with open(version_file, "w") as f:
        f.write(str(version))

    # Afficher la version actuelle
    print(f"Version actuelle du modèle : {version}")