...
def preprocess_fold_data(tr_df: pd.DataFrame, va_df: pd.DataFrame, te_df: pd.DataFrame, 
    ...
    y_train_final = to_categorical(y_train_final, num_classes=n_classes) # Convertir en one-hot encoding
    y_va_seq = to_categorical(y_va_seq, num_classes=n_classes) # Convertir en one-hot encoding
    y_te_seq = to_categorical(y_te_seq, num_classes=n_classes) # Convertir en one-hot encoding
    ...
    # print("threshold_info_for_fold:\n", threshold_info_for_fold)

    # #count n_classes in y_train_final
    # n_0 = np.sum(y_train_final == 0)
    # n_1 = np.sum(y_train_final == 1)
    # n_2 = np.sum(y_train_final == 2)
    # print(f"Training data class distribution: Class 0: {n_0}, Class 1: {n_1}, Class 2: {n_2}, Total: {len(y_train_final)}")

    # avec du one-hot encoding, on doit compter les classes différemment
    class_counts = y_train_final.sum(axis=0).astype(int)
    print(f"Training data class distribution: Class 0: {class_counts[0]}, Class 1: {class_counts[1]}, Class 2: {class_counts[2]}, Total: {len(y_train_final)}")


    input("Press Enter to continue after checking the preprocessed data...")
