...
def diagnose_data_quality(X_train, y_train, X_val, y_val, split_num):
    ...
    # Gérer y_train et y_val en one-hot ou pas
    if y_train.ndim == 2 and y_train.shape[1] == config.N_CLASSES:
        y_train_labels = np.argmax(y_train, axis=1)
    else:
        y_train_labels = y_train

    if y_val.ndim == 2 and y_val.shape[1] == config.N_CLASSES:
    ...
    # Vérifier l'équilibre minimal
    min_class_count = min(counts_train)
    if min_class_count < 5:  # seuil configurable ?
...
def train_single_fold(args):
    ...
    try:
        ...
        # Class weights avec gestion d'erreur et support one-hot
        try:
            # Vérifie si y_train_final est one-hot
            if y_train_final.ndim == 2 and y_train_final.shape[1] == config.N_CLASSES:
                y_train_for_weights = np.argmax(y_train_final, axis=1)
            else:
                y_train_for_weights = y_train_final

            unique_classes = np.unique(y_train_for_weights)

            if len(unique_classes) < config.N_CLASSES:
                print(f"Warning: Not all classes present in training data for split {split_num}. Using uniform weights.")
                class_weights = None
            else:
                cw = compute_class_weight('balanced', classes=unique_classes, y=y_train_for_weights)
        ...
        # Metrics
        # Transforme y_va_seq / y_te_seq en labels entiers si one-hot
        if y_va_seq.ndim == 2 and y_va_seq.shape[1] == config.N_CLASSES:
            y_va_true = np.argmax(y_va_seq, axis=1)
        else:
            y_va_true = y_va_seq

        if y_te_seq.ndim == 2 and y_te_seq.shape[1] == config.N_CLASSES:
            y_te_true = np.argmax(y_te_seq, axis=1)
        else:
            y_te_true = y_te_seq

        # Calcul des métriques
        val_metrics = evaluation.calculate_classification_metrics(y_va_true, y_pred_va)
        test_cls_metrics = evaluation.calculate_classification_metrics(y_te_true, y_pred_te)
        ...
        try:
            if len(te_df_processed) > config.TIMESTEPS and 'ret_future' in te_df_processed.columns:
                ...
                y_te_true, y_pred_te, returns_for_pnl_test, config.PRED_HORIZON)
