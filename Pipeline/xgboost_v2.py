...
# Initialisation du classifieur avec les hyperparamètres spécifiés
model = xgb.XGBClassifier(
    objective='multi:softmax',
    num_class=3,
    n_estimators=800,  # Mis à jour
    max_depth=6,  # Mis à jour
    learning_rate=0.05,  # Mis à jour
    colsample_bytree=0.7,  # Ajouté
    gamma=0,  # Ajouté
    min_child_weight=1,  # Ajouté
    reg_lambda=1,  # Ajouté
    subsample=0.85,  # Ajouté
...
# Calcul du F1-score sur l'ensemble d'entraînement
y_train_pred = model.predict(X_train)
f1_train = f1_score(y_train, y_train_pred, average='weighted')
print(f"F1-score (train, weighted): {f1_train:.4f}")
...
print(classification_report(y_test, y_pred, target_names=['Baisse (0)', 'Neutre (1)', 'Hausse (2)'], zero_division=np.nan))
