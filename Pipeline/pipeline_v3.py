...
def main():
    ...
    print(f"y_pred_proba: {y_pred_proba[-20:]}")  # Affiche les 5 dernières prédictions de probabilité

    # Définir un écart minimum entre la meilleure et la deuxième meilleure classe
    ecart_min = 0.15  # La meilleure classe doit avoir au moins x% de plus

    y_pred = []
    for proba in y_pred_proba:
        sorted_proba = np.sort(proba)[::-1]  # Tri décroissant
        if sorted_proba[0] - sorted_proba[1] >= ecart_min:
            y_pred.append(np.argmax(proba))
        else:
            y_pred.append(0)  # Prédiction incertaine
