...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(df_main)):
        ...
        model = lstm_model_v2(input_shape=(SEQUENCE_LENGTH, len(final_features)), n_classes=N_CLASSES)
    ...
    train_gen_final = TimeseriesGenerator(X_train_final_scaled, y_train_final, length=SEQUENCE_LENGTH, batch_size=BATCH_SIZE)
    test_gen_final = TimeseriesGenerator(X_test_final_scaled, y_test_final, length=SEQUENCE_LENGTH, batch_size=BATCH_SIZE)
