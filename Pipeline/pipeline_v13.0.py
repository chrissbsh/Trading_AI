...
def main(debug_on, feature_selection_on, fine_tune_on=True):
    ...
    log_filename = os.path.join(OPTUNA_DIR, f"optuna_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    horizons_to_test = [1, 2, 3, 4, 5, 6, 7] # <--- DÉFINISSEZ ICI LES HORIZONS À TESTER
    ...
    for current_pred_horizon in horizons_to_test:
        ...
        output_csv_path = os.path.join(PREDICTION_SAVE_DIR, f"predictions_optuna_{current_pred_horizon}_jours.csv")
