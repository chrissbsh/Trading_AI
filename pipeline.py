...
def run_training_pipeline(model_name="lstm", feature_selection=False):
    """
    ...
    model_builder_fn = model_architectures.AVAILABLE_MODELS.get(model_name)
    if not model_builder_fn:
        raise ValueError(f"Model architecture '{model_name}' not found in model_architectures.py")

    wf_results_df, best_model_set = training.train_walk_forward(
        df_roll.copy(), # Use a copy
    ...
    training.save_best_model(best_model_set, version=config.MODEL_VERSION)
...
def run_holdout_evaluation(df_holdout_original: pd.DataFrame, best_model_set_from_training):
    ...
    df_holdout['ret_future'] = preprocessing.compute_future_return(
    ...
    df_holdout['target'] = preprocessing.label_with_sliding_thresholds(
    ...
    # 3. Create sequences
    X_hd_seq, y_hd_seq_true = preprocessing.create_sequences(X_hd_scaled, y_hd_true, timesteps)
    ...
    # y_hd_seq_true corresponds to df_holdout['target'].iloc[timesteps:]
    returns_for_pnl_holdout = df_holdout['ret_future'].iloc[timesteps:]
...
if __name__ == "__main__":
    ...
    selected_model_arch = "lstm" # or "gru" if you implement it
    
    wf_results, best_set = run_training_pipeline(model_name=selected_model_arch)
